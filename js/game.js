...
function drawSettings() {
    ...
    if(inCat){
        ...
        if(SETTINGS_CATS[settingsCat].label==='DATA MANAGEMENT'&&_dataMsg&&performance.now()-_dataMsgAt<2500)
...
function backupStats() {
    try {
        ...
        _dataMsg='BACKUP SAVED'; _dataMsgAt=performance.now();
    } catch { _dataMsg='BACKUP FAILED'; _dataMsgAt=performance.now(); }
}
...
_restoreInp.addEventListener('change',()=>{
    ...
    rd.onload=()=>{
        try {
            ...
            _dataMsg='STATS RESTORED'; _dataMsgAt=performance.now();
        } catch { _dataMsg='INVALID FILE'; _dataMsgAt=performance.now(); }
    };
    rd.onerror=()=>{ _dataMsg='READ FAILED'; _dataMsgAt=performance.now(); };
    rd.readAsText(f);
});
